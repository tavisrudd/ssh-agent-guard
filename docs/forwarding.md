...
This is surfaced as `is_forwarded_session` in log events (used for
confirmation method selection, not policy matching) and is distinct
from `is_forwarded` (which specifically means the agent *socket* is
forwarded via session-bind).
